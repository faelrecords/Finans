  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        {!['pie', 'donut', 'scoreboard', 'table', 'heatmap'].includes(w.chart_type) && <div className="field"><label className="label meta-toggle-label"><span>Cor dinâmica por meta</span><button type="button" className={`switch ${w.dynamic_color ? 'on' : ''}`} onClick={() => set('dynamic_color', !w.dynamic_color)}><i /></button></label>{w.dynamic_color && <div className="meta-grid mt-2"><div className="field meta-full"><label className="label">Meta</label><input className="input" type="number" step="0.01" placeholder="Ex: 100" value={w.goal_value ?? ''} onChange={e => set('goal_value', e.target.value === '' ? '' : Number(e.target.value))} /></div><div className="field"><label className="label">Abaixo da meta</label><input className="input color-field" type="color" value={w.color_below || '#ff8078'} onChange={e => set('color_below', e.target.value)} /></div><div className="field"><label className="label">Na meta</label><input className="input color-field" type="color" value={w.color_on || '#ffb84d'} onChange={e => set('color_on', e.target.value)} /></div><div className="field"><label className="label">Acima da meta</label><input className="input color-field" type="color" value={w.color_above || '#30d173'} onChange={e => set('color_above', e.target.value)} /></div><div className="hint meta-full">"Na meta" = valor exatamente igual ao número acima. Abaixo e acima são aplicados para qualquer desvio.</div></div>}</div>}
